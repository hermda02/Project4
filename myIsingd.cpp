...
void Metropolis(int, int, double, vec&, vec&);
//Write the results
void WritetoFile(int, int, double,vec,vec);
...
int main(int argc, char* argv[]){
	...
	vec EnergyProb = zeros<mat>(100000);
	// Time to Monte the Carlo
	Metropolis(NSpin,MCcycles,Temperature,ExpectationValues,EnergyProb);
	// Then write the expectation values found to the file
	WritetoFile(NSpin,MCcycles,Temperature,ExpectationValues,EnergyProb);
	}
	ofile.close();
	// }
	return 0;
...
void Metropolis(int NSpin, int MCcycles, double Temperature, vec &ExpectationValues, vec &EnergyProb){
	...
	for (int cycles=1; cycles <= MCcycles; cycles++){
		...
		if (cycles >= 1000001){
			EnergyProb(cycles-1000001) = ExpectationValues(0)/cycles;
		} 
	}
	//cout << count << endl;
}
...
void WritetoFile(int NSpin, int MCcycles, double Temperature,vec ExpectationValues, vec EnergyProb){
	...
	ofile << "Energy Variance:" << Evariance << endl;
	ofile << "Energy" << endl;
	for (int i=0;i<=99999;i++){
		double printing = EnergyProb(i)/NSpin/NSpin;
		ofile << setprecision(8) << printing << endl;
	}
